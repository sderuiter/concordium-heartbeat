...
class Heartbeat:
    def __init__(self, grpcclient: GRPCClient, tooter: Tooter, mongodb: MongoDB, TESTNET=False):
        self.grpcclient = grpcclient
        self.tooter = tooter
        self.mongodb = mongodb
        self.TESTNET = TESTNET
        self.db:Dict[Collections, Collection] = self.mongodb.mainnet if not self.TESTNET else self.mongodb.testnet
                ...
                if effects.account_transfer:
                    ...
                    for effect in update_effects:
                        if effect.interrupted:
                            result.list_of_contracts_involved.append({'address': effect.interrupted.address})

                        elif effect.resumed:
                            result.list_of_contracts_involved.append({'address': effect.resumed.address})
                            ...
                            if type(effect.transferred.sender) == CCD_ContractAddress:
                                result.list_of_contracts_involved.append({'address': effect.transferred.sender})
                            
                            if type(effect.transferred.receiver) == CCD_ContractAddress:
                                result.list_of_contracts_involved.append({'address': effect.transferred.receiver})

        return result       

    def index_transfer_and_all(self, tx: CCD_BlockItemSummary, result: ClassificationResult, block_info: CCD_BlockInfo):
    ...
    def index_contract(self, tx: CCD_BlockItemSummary, result: ClassificationResult, contract: CCD_ContractAddress, block_info: CCD_BlockInfo):
    ...
    def add_back_updated_modules_to_queue(self, current_block_to_process: CCD_BlockInfo):
        ...
        self.queues[Queue.updated_modules] = list(set(self.queues[Queue.updated_modules]))
        # for module_ref in self.existing_source_modules.keys():
        for module_ref in self.queues[Queue.updated_modules]:
            try:
                results = self.get_module_metadata(current_block_to_process.hash, module_ref)
            except:
                results = {'module_name': '', 'methods': []}
            module = {
            "_id": module_ref,
            "module_name": results['module_name'] if 'module_name' in results.keys() else None,
            "methods": results['methods'] if 'methods' in results.keys() else None,
            "contracts": list(self.existing_source_modules[module_ref])
            }
            self.queues[Queue.modules].append(ReplaceOne({"_id": module_ref }, module , upsert=True))

    def generate_indices_based_on_transactions(self, 
        ...
        for tx in transactions:
            ...
            self.queues[Queue.involved_all].append(ReplaceOne({"_id": dct_transfer_and_all["_id"] }, dct_transfer_and_all, upsert=True))
            
            # only store tx in this collection if it's a transfer
            if result.accounts_involved_transfer:
                self.queues[Queue.involved_transfer].append(ReplaceOne({"_id": dct_transfer_and_all["_id"] }, dct_transfer_and_all, upsert=True))
            ...
            if result.contracts_involved:
                for contract in result.list_of_contracts_involved:
                    index_contract = self.index_contract(tx, result, contract['address'], block_info)
                    
                    try:
                        instance_info = self.grpcclient.get_instance_info(index_contract['index'], index_contract['subindex'], block_info.hash)
                        ...
                        self.queues[Queue.instances].append(ReplaceOne({"_id": index_contract["contract"] }, instance_info, upsert=True))

                        
                        if not _source_module in self.existing_source_modules.keys():
                            self.existing_source_modules[_source_module] = set()

                        self.existing_source_modules[_source_module].add(index_contract["contract"])
                        self.queues[Queue.updated_modules].append(_source_module)
                        
                        index_contract.update({"source_module": _source_module})

                        self.queues[Queue.involved_contract].append(ReplaceOne({"_id": index_contract["_id"] }, index_contract, upsert=True))
                    except:
                        console.log(f"block or instance not found for {index_contract}...")

    def lookout_for_account_transaction(self, block_info: CCD_BlockInfo, tx: CCD_BlockItemSummary):
        if tx.account_transaction:
            if tx.account_transaction.effects.data_registered:
                query = {"_id":  "last_known_data_registered" }
                self.db[Collections.helpers].replace_one(query, {
    ...
    def add_block_and_txs_to_queue(self, block_info: CCD_BlockInfo):
        ...
        if block_info.transaction_count > 0:
            ...
            block: CCD_Block = self.grpcclient.get_block_transaction_events(block_info.hash)
            ...
            for tx in block.transaction_summaries:
                ...
                self.queues[Queue.transactions].append(ReplaceOne({"_id": tx.hash}, replacement=json_tx, upsert=True))
                # self.lookout_for_account_transaction(block_info, tx)

            self.generate_indices_based_on_transactions(block.transaction_summaries, block_info)
            
        self.queues[Queue.blocks].append(ReplaceOne({"_id": block_info.hash}, replacement=json_block_info, upsert=True))
    ...
    def lookout_for_end_of_day(self, current_block_to_process: CCD_BlockInfo):
        ...
        if (current_block_to_process.slot_time.time() >= end_of_day_timeframe_start) and \
            ...
            previous_block_info = self.grpcclient.get_finalized_block_at_height(previous_block_height)

            if current_block_to_process.slot_time.day != previous_block_info.slot_time.day:

                start_of_day0 = previous_block_info.slot_time.replace(hour=0, minute=0,second=0,microsecond=0)
                start_of_day1 = previous_block_info.slot_time.replace(hour=0, minute=1,second=59,microsecond=999999)
            
                start_of_day_blocks = list(self.db[Collections.blocks].find(filter={
                    '$and': [
    ...
    def lookout_for_payday(self, current_block_to_process: CCD_BlockInfo):
        ...
        if (current_block_to_process.slot_time.time() > payday_timeframe_start) and \
            ...
            special_events = self.grpcclient.get_block_special_events(current_block_to_process.hash)
            found = False
    ...
    def log_error_in_mongo(self, e, current_block_to_process: CCD_BlockInfo):
        ...
        mongodb.self.db[Collections.helpers].replace_one(query, {
        ...
        while True:
            try:
                if len(self.queues[Queue.blocks]) > 0:
                    update_ = True
                    heartbeat_last_processed_block = CCD_BlockInfo(**self.queues[Queue.blocks][-1]._doc)
                    result = self.db[Collections.blocks].bulk_write(self.queues[Queue.blocks])
                    if len(self.queues[Queue.block_heights]) == 1:
                        console.log (f"Sent to Mongo  : {self.queues[Queue.block_heights][0]:,.0f}")    
                    else:
                        console.log (f"Sent to Mongo   : {self.queues[Queue.block_heights][0]:,.0f} - {self.queues[Queue.block_heights][-1]:,.0f}")
                    console.log (f"B:  {len(self.queues[Queue.blocks]):5,.0f} | M {result.matched_count:5,.0f} | Mod {result.modified_count:5,.0f} | U {result.upserted_count:5,.0f}")
                ...
                if len(self.queues[Queue.transactions]) > 0:
                    result = self.db[Collections.transactions].bulk_write(self.queues[Queue.transactions])
                    console.log (f"T:  {len(self.queues[Queue.transactions]):5,.0f} | M {result.matched_count:5,.0f} | Mod {result.modified_count:5,.0f} | U {result.upserted_count:5,.0f}")
                    self.queues[Queue.transactions] = []

                if len(self.queues[Queue.involved_all]) > 0:
                    result = self.db[Collections.involved_accounts_all].bulk_write(self.queues[Queue.involved_all])
                    console.log (f"A:  {len(self.queues[Queue.involved_all]):5,.0f} | M {result.matched_count:5,.0f} | Mod {result.modified_count:5,.0f} | U {result.upserted_count:5,.0f}")
                    self.queues[Queue.involved_all] = []
                if len(self.queues[Queue.involved_transfer]) > 0:

                    result = self.db[Collections.involved_accounts_transfer].bulk_write(self.queues[Queue.involved_transfer])
                    console.log (f"Tr: {len(self.queues[Queue.involved_transfer]):5,.0f} | M {result.matched_count:5,.0f} | Mod {result.modified_count:5,.0f} | U {result.upserted_count:5,.0f}")
                    self.queues[Queue.involved_transfer] = []

                if len(self.queues[Queue.involved_contract]) > 0:
                    result = self.db[Collections.involved_contracts].bulk_write(self.queues[Queue.involved_contract])
                    console.log (f"C:  {len(self.queues[Queue.involved_contract]):5,.0f} | M {result.matched_count:5,.0f} | Mod {result.modified_count:5,.0f} | U {result.upserted_count:5,.0f}")
                    self.queues[Queue.involved_contract] = []

                if len(self.queues[Queue.instances]) > 0:
                    result = self.db[Collections.instances].bulk_write(self.queues[Queue.instances])
                    console.log (f"I:  {len(self.queues[Queue.instances]):5,.0f} | M {result.matched_count:5,.0f} | Mod {result.modified_count:5,.0f} | U {result.upserted_count:5,.0f}")
                    self.queues[Queue.instances] = []

                if len(self.queues[Queue.modules]) > 0:
                    result = self.db[Collections.modules].bulk_write(self.queues[Queue.modules])
                    console.log (f"M:  {len(self.queues[Queue.modules]):5,.0f} | M {result.matched_count:5,.0f} | Mod {result.modified_count:5,.0f} | U {result.upserted_count:5,.0f}")
                    self.queues[Queue.modules] = []

                if len(self.queues[Queue.block_per_day]) > 0:
                    result = self.db[Collections.blocks_per_day].bulk_write(self.queues[Queue.block_per_day])
    ...
    async def process_blocks(self):
        ...
        while True:
            if len(self.finalized_block_infos_to_process) > 0:
                ...
                self.existing_source_modules:dict[CCD_ModuleRef,set] = {x['_id']:set(x['contracts']) for x in list(result)}
                self.queues[Queue.updated_modules] = []

                start = dt.datetime.now()
                while len(self.finalized_block_infos_to_process) > 0:
            
                    current_block_to_process: CCD_BlockInfo = self.finalized_block_infos_to_process.pop(0)
                    console.log (f"Processing {current_block_to_process.height:,.0f} with {current_block_to_process.transaction_count:4,.0f} transaction(s)...")
                    try:
                ...
                console.log(f"Spent {duration.total_seconds():,.0f} sec on {len(self.queues[Queue.transactions]):,.0f} txs.")
    ...
    async def get_finalized_blocks(self):
        ...
        while True:
            request_counter = 0
            result = self.db[Collections.helpers].find_one({"_id": "heartbeat_last_processed_block"})
            ...
            while not (last_requested_block_not_finalized) and (request_counter < MAX_BLOCKS_PER_RUN) and not block_to_request_in_queue:
                ...
                block_to_request_in_queue = heartbeat_last_processed_block_height in [x.height for x in self.finalized_block_infos_to_process]

                # we haven't previously requested this block
                if not block_to_request_in_queue:
                    try:
                        finalized_block_info_at_height = self.grpcclient.get_finalized_block_at_height(heartbeat_last_processed_block_height)
                    except:
            ...
            if len (self.finalized_block_infos_to_process) > 0:
                if len (self.finalized_block_infos_to_process)  == 1:
                    console.log (f"Block retrieved: {self.finalized_block_infos_to_process[0].height:,.0f}")
                else:

                    console.log (f"Blocks retrieved: {self.finalized_block_infos_to_process[0].height:,.0f} - {self.finalized_block_infos_to_process[-1].height:,.0f}")
            await asyncio.sleep(1)

    def add_end_of_day_to_queue(self, date_string: str, start_block: CCD_BlockInfo, end_block: CCD_BlockInfo):
        '''
        ...
        self.queues[Queue.block_per_day].append(ReplaceOne({"_id": date_string }, dd, upsert=True))
    ...
    def create_block_per_day(self):
        ...
        for index, date in enumerate(date_range):
            ...
            if index == 0:
                if TESTNET:
                    start_of_day0 = date.replace(hour=10, minute=0,second=0,microsecond=0)
                    start_of_day1 = date.replace(hour=10, minute=1,second=59,microsecond=999999)
                else:
                    start_of_day0 = date.replace(hour=6, minute=0,second=0,microsecond=0)
                    start_of_day1 = date.replace(hour=6, minute=1,second=59,microsecond=999999)

            else:
                start_of_day0 = date.replace(hour=0, minute=0,second=0,microsecond=0)
                start_of_day1 = date.replace(hour=0, minute=1,second=59,microsecond=999999)
            

            end_of_day0 = date.replace(hour=23, minute=58,second=0,microsecond=0)
            end_of_day1 = date.replace(hour=23, minute=59,second=59,microsecond=999999)
            ...
            end_of_day_blocks = list(self.db[Collections.blocks].find(filter={
                '$and': [
    ...
    def create_index(self, collection: Collections, key: str, direction):
        ...
        print (f"Reponse for index creation on collection '{collection.value}' for key '{key}': {response}.")
    ...
    def create_mongodb_indices(self):
        ...
        for collection in Collections:
            ...
            if collection in [Collections.involved_accounts_all, Collections.involved_accounts_transfer]:
...
def main():
    ...
    if TESTNET:
        grpcclient = GRPCClient(host=TESTNET_IP, port=TESTNET_PORT)
        console.log(f"Connecting on {TESTNET_IP}:{TESTNET_PORT}")
    else:
        grpcclient = GRPCClient(host=MAINNET_IP, port=MAINNET_PORT)
