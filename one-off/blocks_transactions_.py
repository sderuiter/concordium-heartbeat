...
from ccdefundamentals.GRPCClient.queries import *
...
from ccdefundamentals.mongodb import MongoDB, TestNetCollections
import os
import json
import dateutil
...
ENVIRONMENT = (
    "dev" if API_TOKEN == "5456222180:AAF6RYi_zNziRi4DrE_GCffqHMrP5EHVR-A" else "prod"
)
...
class Heartbeat:
    ...
    def loop(self, start, end, report_interval=1_000):
        ...
        for block_height in track(range(start, end)):
            ...
            if (self.block_counter / report_interval) == int(
                self.block_counter / report_interval
            ):
                duration = (dt.datetime.now() - start_time).total_seconds()

                print(
                    f"# Block group end: {block_height:8,.0f}, # Transactions: {(self.tx_counter - prev_tx_count):4,.0f}, Process Time: {duration:3,.3f}s"
                )
                start_time = dt.datetime.now()
...
def instances(grpcclient: GRPCClient, testnet=False):
    ...
    for index, instance in enumerate(il):
        ii = grpcclient.get_instance_info(
            instance["index"], instance["subindex"], block_hash
        )
        ...
        if ii.v0.source_module != "":
            ...
            if testnet:
                query = {"_id": jj["_id"]}
                mongodb.testnet[TestNetCollections.instances].replace_one(
                    query, jj, upsert=True
                )
